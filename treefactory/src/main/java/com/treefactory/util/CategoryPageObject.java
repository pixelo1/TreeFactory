...
public class CategoryPageObject {
	...
	public CategoryPageObject(long page, long perPageNum) {
		this.page = page;
		...
		perGroupPageNum = 10;
		System.out.println("CategoryPageObject(페이지,페이지 당 글수)");
	}
	...
	public static CategoryPageObject getInstance(HttpServletRequest request) throws Exception {

		// 페이지 처리를 위한 프로그램
		// 페이지 처리를 위한 객체 사용
		CategoryPageObject categoryPageObject = new CategoryPageObject();
		// 페이지에 대한 정보를 받는다.
		// page는 jsp에서 기본객체로 사용하고 있다. -> 페이지의 정보가 담겨져 있다.
		String strPage = request.getParameter("page");
		// 넘어오는 페이지가 있는 경우는 넘어오는 페이지를 현재 페이지로 셋팅. 그렇지 않으면 1이 셋팅된다.
		if(strPage != null && !strPage.equals("")) categoryPageObject.setPage(Integer.parseInt(strPage));
		// 한페이지에 표시할 데이터의 수를 받는다.
		String strPerPageNum = request.getParameter("perPageNum");
		// 한 페이지당 표시할 데이터의 수가 안넘어오면 10으로 셋팅된다. 넘어오면 넘어 오는 데이터를 사용한다.
		if(strPerPageNum != null && !strPerPageNum.equals("")) categoryPageObject.setPerPageNum(Integer.parseInt(strPerPageNum));
		
		// 검색을 위한 데이터 전달
		categoryPageObject.setKey(request.getParameter("key"));
		categoryPageObject.setWord(request.getParameter("word"));
		
		// CategoryPageObject - 확인
		System.out.println("CategoryPageObject.getInstance() [categoryPageObject = " + categoryPageObject + " ]");

		return categoryPageObject;
	}
	
	// multi 
	public static CategoryPageObject getMultiInstance(MultipartRequest multi) throws Exception {
		...
		// 검색을 위한 데이터 전달
		categoryPageObject.setKey(multi.getParameter("key"));
		categoryPageObject.setWord(multi.getParameter("word"));
	...
	public static CategoryPageObject getInstance(HttpServletRequest request, String pageName, String perPageNumName) throws Exception {
		...
		// CategoryPageObject - 확인
		System.out.println("CategoryPageObject.getInstance() [pageName = " + pageName + " ]");
		System.out.println("CategoryPageObject.getInstance() [perPageNumName = " + perPageNumName + " ]");
		System.out.println("CategoryPageObject.getInstance() [categoryPageObject = " + categoryPageObject + " ]");
	...
	public String toString() {
		return "CategoryPageObject [page=" + page + ", perPageNum=" + perPageNum + ", startRow=" + startRow
				...
				+ ", word=" + word + ", period=" + period + ", no=" + no + ", category=" + category + ", accepter="
				+ accepter + ", acceptMode=" + acceptMode + "]";
	}
